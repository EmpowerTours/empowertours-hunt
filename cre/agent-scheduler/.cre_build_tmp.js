...
var callAgent = (token) => (sendRequester, config) => {
  const resp = sendRequester.sendRequest({
    ...
    body: new TextEncoder().encode(JSON.stringify({ dryRun: config.dryRun }))
  }).result();
  if (!ok(resp)) {
...
var onCronTrigger = (runtime) => {
  ...
  const summary = httpClient.sendRequest(runtime, callAgent(token.value), ConsensusAggregationByFields({
    ran: median,
    acted: median
  }))(runtime.config).result();
  runtime.log(`cota agent: ${String(summary.ran)} leash(es) considered, ${String(summary.acted)} acted on${runtime.config.dryRun ? " (dry run)" : ""}`);
